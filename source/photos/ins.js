...
（[
  ...
  function（module，exports）{
    ...
    var  initPhotoSwipeFromDOM  =  function  initPhotoSwipeFromDOM（gallerySelector）{
      ...
      为（VAR我=  0，L =  galleryElements。长度 ;我<升;我++）{
        galleryElements [i]中。setAttribute（' data-pswp-uid '，i +  1）;
        galleryElements [i]中。onclick  = onThumbnailsClick;
      }

      //解析URL并打开图库，如果它包含＃＆pid = 3＆gid = 1
      var hashData =  photoswipeParseHash（）;
      如果（hashData。PID  &&  hashData。GID）{
        openPhotoSwipe（hashData。PID，galleryElements [ hashData。GID  -  1 ]，真，真）;
      }
    };
  ...
  function（module，exports）{
    ...
    / * WEBPACK VAR INJECTION * /
    （function（global）{
      模块。exports  = inViewport;

      var instances = [];
      var supportsMutationObserver =  typeof  global。MutationObserver  ===  ' function ' ;

      函数 inViewport（elt，params，cb）{
        var opts = {
          容器： 全球。文件。身体，
          偏移量： 0
        };
        ...
        var container =  opts。container  =  params。容器 ||  选择。容器 ;
        var offset =  opts。offset  =  params。偏移 ||  选择。偏移 ;

        为（VAR我=  0 ;我<  实例。长度 ;我++）{
          如果（实例[i]中。容器 ===容器）{
            返回实例[i]。isInViewport（elt，offset，cb）;
          }
        }

        返回实例[
          实例。push（createInViewport（container））-  1
      ...
      var  contains  =  function（）{
        ...
        回归 全球。文件。documentElement。compareDocumentPosition  ？
          function（a，b）{
            回来 !! （一个。compareDocumentPosition（B） ＆ 16）;
          } ：
          全球的。文件。documentElement。包含 ？
          function（a，b）{
            返回 a ！== b &&（a。包含 ？ a。包含（b）： false）;
          } ：
          function（a，b）{
            而（B =  b。parentNode）{
              if（b === a）{
      ...
      function  createInViewport（container）{
        ...
        var scrollContainer = container ===  global。文件。身体 ？ 全球 ：容器;
        VAR debouncedCheck =  反跳（手表。checkAll（watchInViewport），15）;
        ...
        if（supportsMutationObserver）{
          observeDOM（手表，容器，debouncedCheck）;
        }

        //故障安全检查，我们每200ms检查一次可见图像
        // usecase：包含元素的隐藏父级
        //当父母变得可见时，我们没有孩子的事件
        //变得可见
        ...
        function  isVisible（elt，offset）{
          如果（！包含（全球。文件。documentElement，ELT）||  ！包含（全球。文件。documentElement，集装箱））{
            返回 false ;
          }

          //检查元素是否可见
          // https://github.com/jquery/jquery/blob/740e190223d19a114d5373758127285d14d6b71e/src/css/hiddenVisibleSelectors.js
          如果（！ELT。offsetWidth  ||  ！ELT。的offsetHeight）{
            返回 false ;
          }
          ...
          如果（集装箱===  全球。文件。体）{
            viewport = {
              顶部： -偏移，
              左： -偏移，
              对： 全球。文件。documentElement。clientWidth  + offset，
              底部： 全球。文件。documentElement。clientHeight  + offset
            };
          } else {
            var containerRect =  container。getBoundingClientRect（）;
            viewport = {
              顶部： containerRect。顶部 -偏移，
              left ： containerRect。左 -偏移，
              右： containerRect。右 +偏移，
              bottom ： containerRect。底部 +偏移
            };
          }
